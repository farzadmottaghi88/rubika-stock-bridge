...
function buildTradePlan(row) {
  ...
  const volatilityPct30d = Number(row.volatilityPct30d);
  ...
  if (support >= entry) {
    return {
      ...
      riskReward: null,
      signalEligible: false,
      signalReason: "حمایت زیر قیمت ورود نیست"
    };
  }

  const volatilityBuffer = Number.isFinite(volatilityPct30d) && volatilityPct30d > 0
    ? clamp(volatilityPct30d * 0.35, 0.5, 3)
    : 1;
  const stopPercent = Math.max(1, volatilityBuffer);
  const stop = support * (1 - stopPercent / 100);
  ...
  const realFlow = Number(row.realMoneyFlowRatio) >= 1;
  const buyerPower = Number(row.buyerPower) > 1;
  const validRisk = Number.isFinite(riskPercent) && riskPercent > 0;
  const validTarget = Number.isFinite(target) && target > entry;
  ...
  let signalReason = "فیلتر سیگنال کامل نشده";
  if (!validRisk) signalReason = "ریسک محاسبه‌پذیر نیست";
  else if (!validTarget) signalReason = "هدف معتبر بالاتر از ورود موجود نیست";
  else if (riskReward < 2) signalReason = "نسبت R/R کمتر از ۲ است";
  else if (!strictVolume) signalReason = "حجم کمتر از ۳ برابر میانگین ۳۰روزه است";
  else if (!positiveTrend) signalReason = "SMA20 بالاتر از SMA50 نیست";
  else if (!liquid) signalReason = "نقدشوندگی کافی نیست";
  else if (!realFlow) signalReason = "ورود پول حقیقی تأیید نشده";
  else if (!buyerPower) signalReason = "قدرت خریدار تأیید نشده";
  else if (signalEligible) signalReason = "فیلتر حجم، روند، نقدشوندگی، جریان پول و R/R تأیید شد";
