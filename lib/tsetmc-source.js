const USER_AGENT =
  "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/120 Safari/537.36";

const DEFAULT_SNAPSHOT_URL =
  "https://raw.githubusercontent.com/farzadmottaghi88/rubika-stock-bridge/market-data/live-market.json";
const SNAPSHOT_MAX_AGE_MS = 15 * 60 * 1000;

const MARKETWATCH_URLS = [
  "http://old.tsetmc.com/tsev2/data/MarketWatchInit.aspx?h=0&r=0",
  "http://www.tsetmc.com/tsev2/data/MarketWatchInit.aspx?h=0&r=0",
  "http://members.tsetmc.com/tsev2/data/MarketWatchInit.aspx?h=0&r=0"
];
const CLIENTTYPE_URLS = [
  "http://old.tsetmc.com/tsev2/data/ClientTypeAll.aspx",
  "http://www.tsetmc.com/tsev2/data/ClientTypeAll.aspx",
  "http://members.tsetmc.com/tsev2/data/ClientTypeAll.aspx"
];

const CDN_MARKETWATCH_URL =
  "https://cdn.tsetmc.com/api/ClosingPrice/GetMarketWatch" +
  "?market=0&paperTypes[0]=1&paperTypes[1]=2&paperTypes[2]=3" +
  "&paperTypes[3]=4&paperTypes[4]=5&paperTypes[5]=6" +
  "&paperTypes[6]=7&paperTypes[7]=8&paperTypes[8]=9" +
  "&showTraded=false&withBestLimits=false&hEven=0&RefID=0";

function num(value) {
  if (value === null || value === undefined || value === "") return null;
  const n = Number(value);
  return Number.isFinite(n) ? n : null;
}

function parseLegacyMarketWatch(raw) {
  const parts = String(raw).split("@");
  if (parts.length < 5) return { rows: [], bestLimits: [], refId: null };

  const states = parts[2] || "";
  const bestLimitsRaw = parts[3] || "";
  const refId = num(parts[4]);

  const rows = states
    .split(";")
    .map(line => line.trim())
    .filter(Boolean)
    .map(line => line.split(","))
    .filter(a => a.length >= 23)
    .map(a => ({
      insCode: a[0],
      isin: a[1],
      symbol: a[2],
      name: a[3],
      heven: num(a[4]),
      open: num(a[5]),
      close: num(a[6]),
      last: num(a[7]),
      tradeCount: num(a[8]),
      volume: num(a[9]),
      value: num(a[10]),
      low: num(a[11]),
      high: num(a[12]),
      yesterday: num(a[13]),
      eps: num(a[14]),
      baseVolume: num(a[15]),
      visitCount: num(a[16]),
      flow: num(a[17]),
      cs: a[18],
      tmax: num(a[19]),
      tmin: num(a[20]),
      z: num(a[21]),
      yval: a[22],
      predicted: num(a[23]),
      buyOpen: num(a[24]),
      changePercent: a[25]
    }));

  const bestLimits = bestLimitsRaw
    .split(";")
    .map(line => line.trim())
    .filter(Boolean)
    .map(line => line.split(","))
    .filter(a => a.length >= 8)
    .map(a => ({
      insCode: a[0],
      level: num(a[1]),
      bidCount: num(a[2]),
      bidPrice: num(a[3]),
      bidVolume: num(a[4]),
      askPrice: num(a[5]),
      askVolume: num(a[6]),
      askCount: num(a[7])
    }));

  return { rows, bestLimits, refId };
}

function unwrapJsonRows(json, keys) {
  if (Array.isArray(json)) return json;
  for (const key of keys) {
    if (Array.isArray(json?.[key])) return json[key];
  }
  return [];
}

function normalizeCdnRow(row) {
  return {
    insCode: String(row.insCode ?? row.inscode ?? row.InsCode ?? ""),
    symbol: row.lVal18AFC ?? row.symbol ?? row.Symbol ?? "",
    name: row.lVal30 ?? row.name ?? row.Name ?? "",
    lastPrice: num(row.pDrCotVal ?? row.pl ?? row.last),
    closePrice: num(row.pClosing ?? row.pc ?? row.close),
    openPrice: num(row.pf ?? row.open),
    yesterdayPrice: num(row.py ?? row.priceYesterday),
    lowPrice: num(row.priceMin ?? row.low),
    highPrice: num(row.priceMax ?? row.high),
    tradeCount: num(row.zTotTran ?? row.tno ?? row.count),
    volume: num(row.qTotTran5J ?? row.tvol ?? row.volume),
    tradeValue: num(row.qTotCap ?? row.tval ?? row.value),
    baseVolume: num(row.baseVol ?? row.bvol),
    eps: num(row.eps ?? row.estimatedEPS),
    pe: num(row.pe),
    sectorPe: num(row.sectorPE),
    heven: num(row.hEven ?? row.heven),
    receivedAt: new Date().toISOString(),
    raw: row
  };
}

function normalizeLegacyRow(row) {
  return {
    insCode: String(row.insCode || ""),
    symbol: row.symbol || "",
    name: row.name || "",
    lastPrice: row.last,
    closePrice: row.close,
    openPrice: row.open,
    yesterdayPrice: row.yesterday,
    lowPrice: row.low,
    highPrice: row.high,
    tradeCount: row.tradeCount,
    volume: row.volume,
    tradeValue: row.value,
    baseVolume: row.baseVolume,
    eps: row.eps,
    pe: null,
    sectorPe: null,
    heven: row.heven,
    flow: row.flow,
    receivedAt: new Date().toISOString(),
    raw: row
  };
}

function parseClientType(raw) {
  return String(raw)
    .split(";")
    .map(x => x.trim())
    .filter(Boolean)
    .map(line => line.split(","))
    .filter(a => a.length >= 9)
    .map(a => ({
      insCode: String(a[0]),
      naturalBuyCount: num(a[1]),
      legalBuyCount: num(a[2]),
      naturalBuyVolume: num(a[3]),
      legalBuyVolume: num(a[4]),
      naturalSellCount: num(a[5]),
      legalSellCount: num(a[6]),
      naturalSellVolume: num(a[7]),
      legalSellVolume: num(a[8])
    }));
}

function mergeClientType(rows, clientRows) {
  const map = new Map(clientRows.map(x => [x.insCode, x]));
  return rows.map(row => {
    const c = map.get(row.insCode);
    if (!c) return row;

    const realBuy = Number(c.naturalBuyVolume) || 0;
    const realSell = Number(c.naturalSellVolume) || 0;
    const realBuyValue = realBuy * (Number(row.lastPrice) || 0);
    const realSellValue = realSell * (Number(row.lastPrice) || 0);
    const realMoneyFlowRatio = realSellValue > 0 ? realBuyValue / realSellValue : null;
    const buyerPower =
      c.naturalBuyCount > 0 && c.naturalSellCount > 0
        ? (realBuy / c.naturalBuyCount) / (realSell / c.naturalSellCount)
        : null;

    return {
      ...row,
      naturalBuyCount: c.naturalBuyCount,
      legalBuyCount: c.legalBuyCount,
      naturalBuyVolume: realBuy,
      legalBuyVolume: c.legalBuyVolume,
      naturalSellCount: c.naturalSellCount,
      legalSellCount: c.legalSellCount,
      naturalSellVolume: realSell,
      legalSellVolume: c.legalSellVolume,
      realMoneyFlow: realBuyValue - realSellValue,
      realMoneyFlowRatio,
      buyerPower
    };
  });
}

function mergeBestLimits(rows, limits) {
  const byCode = new Map();
  for (const x of limits) {
    if (!byCode.has(x.insCode)) byCode.set(x.insCode, []);
    byCode.get(x.insCode).push(x);
  }
  return rows.map(row => {
    const levels = (byCode.get(row.insCode) || []).sort((a,b) => (a.level ?? 99) - (b.level ?? 99));
    const l1 = levels[0];
    return {
      ...row,
      bestBidPrice: l1?.bidPrice ?? null,
      bestBidVolume: l1?.bidVolume ?? null,
      bestBidCount: l1?.bidCount ?? null,
      bestAskPrice: l1?.askPrice ?? null,
      bestAskVolume: l1?.askVolume ?? null,
      bestAskCount: l1?.askCount ?? null,
      orderBookLevels: levels
    };
  });
}

async function fetchText(url, accept = "*/*") {
  const response = await fetch(url, {
    headers: { Accept: accept, "User-Agent": USER_AGENT },
    signal: AbortSignal.timeout(8000)
  });
  const raw = await response.text();
  return {
    ok: response.ok,
    status: response.status,
    contentType: response.headers.get("content-type") || "",
    raw
  };
}

async function fetchFirstWorking(urls) {
  const diagnostics = [];
  for (const url of urls) {
    const started = Date.now();
    try {
      const result = await fetchText(url);
      diagnostics.push({
        url,
        ok: result.ok,
        status: result.status,
        chars: result.raw.length,
        elapsedMs: Date.now() - started
      });
      if (result.ok && result.raw.length > 500 && !/General Error Detected/i.test(result.raw)) {
        return { ...result, url, diagnostics };
      }
    } catch (error) {
      diagnostics.push({
        url,
        ok: false,
        error: error instanceof Error ? error.message : "Unknown network error",
        elapsedMs: Date.now() - started
      });
    }
  }
  return { ok: false, raw: "", status: null, contentType: "", diagnostics };
}

async function collectLegacy() {
  const started = Date.now();
  try {
    const [market, client] = await Promise.all([
      fetchFirstWorking(MARKETWATCH_URLS),
      fetchFirstWorking(CLIENTTYPE_URLS)
    ]);

    const parsed = parseLegacyMarketWatch(market.raw);
    const clients = parseClientType(client.raw);
    let rows = parsed.rows.map(normalizeLegacyRow);
    rows = mergeClientType(rows, clients);
    rows = mergeBestLimits(rows, parsed.bestLimits);

    const valid = rows.filter(r =>
      r.insCode && r.symbol && Number.isFinite(Number(r.lastPrice))
    );
    const hevens = valid.map(r => r.heven).filter(Number.isFinite);
    const latestHeven = hevens.length ? Math.max(...hevens) : null;

    return {
      source: "tsetmc-legacy-marketwatch-init",
      reachable: market.ok && client.ok,
      httpStatus: market.status,
      contentType: market.contentType,
      responseChars: market.raw.length,
      recordCount: parsed.rows.length,
      clientRecordCount: clients.length,
      validMarketRows: valid.length,
      blocked: /مسدود|دسترسی شما|General Error Detected/i.test(market.raw),
      latestHeven,
      elapsedMs: Date.now() - started,
      endpoint: market.url || null,
      diagnostics: {
        market: market.diagnostics || [],
        client: client.diagnostics || []
      },
      rows: valid
    };
  } catch (error) {
    return {
      source: "tsetmc-legacy-marketwatch-init",
      reachable: false,
      validMarketRows: 0,
      rows: [],
      elapsedMs: Date.now() - started,
      error: error instanceof Error ? error.message : "Unknown network error"
    };
  }
}



async function collectCdn() {
  const started = Date.now();
  try {
    const result = await fetchText(CDN_MARKETWATCH_URL, "application/json");
    let json = null;
    try { json = JSON.parse(result.raw); } catch {}
    const rawRows = unwrapJsonRows(json, ["marketwatch", "marketWatch", "marketWatchDto"]);
    const rows = rawRows.map(normalizeCdnRow).filter(r =>
      r.insCode && r.symbol && Number.isFinite(Number(r.lastPrice))
    );
    return {
      source: "tsetmc-cdn-marketwatch",
      reachable: result.ok,
      httpStatus: result.status,
      contentType: result.contentType,
      responseChars: result.raw.length,
      recordCount: rawRows.length,
      validMarketRows: rows.length,
      blocked: /مسدود|دسترسی شما|General Error Detected/i.test(result.raw),
      latestHeven: rows.length ? Math.max(...rows.map(r => r.heven).filter(Number.isFinite)) : null,
      elapsedMs: Date.now() - started,
      endpoint: CDN_MARKETWATCH_URL,
      rows
    };
  } catch (error) {
    return {
      source: "tsetmc-cdn-marketwatch",
      reachable: false,
      validMarketRows: 0,
      rows: [],
      elapsedMs: Date.now() - started,
      error: error instanceof Error ? error.message : "Unknown network error"
    };
  }
}



const INSTRUMENT_INFO_URL = "https://cdn.tsetmc.com/api/Instrument/GetInstrumentInfo";

async function fetchInstrumentInfo(insCode) {
  try {
    const result = await fetchText(
      `${INSTRUMENT_INFO_URL}/${encodeURIComponent(insCode)}`,
      "application/json"
    );
    let json = null;
    try { json = JSON.parse(result.raw); } catch {}
    const info = json?.instrumentInfo;
    if (!result.ok || !info) return null;
    const eps = info.eps || {};
    const sector = info.sector || {};
    return {
      eps: num(eps.estimatedEPS ?? eps.epsValue),
      estimatedEPS: num(eps.estimatedEPS),
      sectorPe: num(eps.sectorPE),
      sectorCode: sector.cSecVal ?? null,
      sectorName: sector.lSecVal ?? null
    };
  } catch {
    return null;
  }
}

async function enrichValuation(rows, limit = 20) {
  const candidates = rows
    .filter(r => r.symbol && r.insCode)
    .slice(0, limit);
  const enriched = [];
  for (const row of candidates) {
    const info = await fetchInstrumentInfo(row.insCode);
    if (!info) { enriched.push(row); continue; }
    const eps = Number(info.eps);
    const sectorPe = Number(info.sectorPe);
    const last = Number(row.lastPrice);
    const theoretical = Number.isFinite(eps) && Number.isFinite(sectorPe) ? eps * sectorPe : null;
    const gap = Number.isFinite(theoretical) && Number.isFinite(last) && last > 0
      ? ((theoretical - last) / last) * 100
      : null;
    enriched.push({
      ...row,
      eps: Number.isFinite(eps) ? eps : row.eps,
      sectorPe: Number.isFinite(sectorPe) ? sectorPe : row.sectorPe,
      sectorCode: info.sectorCode,
      sectorName: info.sectorName,
      theoreticalPrice: theoretical,
      peGapPercent: gap,
      valuationSource: "TSETMC InstrumentInfo"
    });
  }
  return enriched;
}

const DAILY_HISTORY_URL = "https://cdn.tsetmc.com/api/ClosingPrice/GetClosingPriceDailyList";

async function fetchDailyHistory(insCode, top = 35) {
  const started = Date.now();
  try {
    const result = await fetchText(
      `${DAILY_HISTORY_URL}/${encodeURIComponent(insCode)}/${top}`,
      "application/json"
    );
    let json = null;
    try { json = JSON.parse(result.raw); } catch {}
    const rows = unwrapJsonRows(json, ["closingPriceDaily"]);
    const history = rows
      .map(r => ({
        date: Number(r.dEven ?? r.date),
        close: num(r.pClosing ?? r.pc ?? r.close),
        last: num(r.pDrCotVal ?? r.pl ?? r.last),
        volume: num(r.qTotTran5J ?? r.tvol ?? r.volume),
        value: num(r.qTotCap ?? r.tval ?? r.value),
        trades: num(r.zTotTran ?? r.tno ?? r.count)
      }))
      .filter(r => Number.isFinite(r.close) && Number.isFinite(r.volume));

    return {
      ok: result.ok,
      status: result.status,
      history,
      elapsedMs: Date.now() - started
    };
  } catch (error) {
    return {
      ok: false,
      status: null,
      history: [],
      elapsedMs: Date.now() - started,
      error: error instanceof Error ? error.message : "Unknown network error"
    };
  }
}

function enrichFromHistory(row, history) {
  const sorted = [...history]
    .filter(x => Number.isFinite(x.close) && Number.isFinite(x.volume))
    .sort((a, b) => b.date - a.date);
  const last30 = sorted.slice(0, 30);
  if (!last30.length) return row;

  const avg30 = last30.reduce((s, x) => s + x.volume, 0) / last30.length;
  const closes20 = last30.slice(0, 20).map(x => x.close);
  const closes50 = sorted.slice(0, 50).map(x => x.close);
  const sma20 = closes20.length ? closes20.reduce((s, x) => s + x, 0) / closes20.length : null;
  const sma50 = closes50.length ? closes50.reduce((s, x) => s + x, 0) / closes50.length : null;
  const prior20 = sorted.slice(1, 21);
  const resistance = prior20.reduce(
    (max, x) => Math.max(max, Number(x.close) || 0), 0
  );
  const support = prior20.reduce(
    (min, x) => Math.min(min, Number(x.close) || Infinity), Infinity
  );
  const returnSeries = sorted.slice(0, 30).slice(0, -1).map((x, i) => {
    const prev = sorted[i + 1]?.close;
    return Number.isFinite(prev) && prev > 0 ? (x.close - prev) / prev : null;
  }).filter(Number.isFinite);
  const meanReturn = returnSeries.length
    ? returnSeries.reduce((s, x) => s + x, 0) / returnSeries.length
    : null;
  const variance = returnSeries.length
    ? returnSeries.reduce((s, x) => s + Math.pow(x - meanReturn, 2), 0) / returnSeries.length
    : null;
  const volatilityPct30d = Number.isFinite(variance) ? Math.sqrt(variance) * 100 : null;
  const last = Number(row.lastPrice);

  return {
    ...row,
    volumeAvg30d: avg30,
    volumeRatio30d: avg30 > 0 ? Number(row.volume) / avg30 : null,
    sma20,
    sma50,
    support20d: Number.isFinite(support) ? support : null,
    resistance20d: resistance || null,
    distanceToSupportPercent: Number.isFinite(last) && support > 0
      ? ((last - support) / last) * 100
      : null,
    distanceToResistancePercent: Number.isFinite(last) && resistance > 0
      ? ((resistance - last) / last) * 100
      : null,
    volatilityPct30d,
    resistanceBreak: Number(row.lastPrice) > resistance && resistance > 0,
    historyDays: sorted.length
  };
}

async function enrichCandidates(rows, limit = 30) {
  const candidates = rows
    .filter(r => r.symbol && Number(r.tradeCount) > 30)
    .filter(r => Number(r.realMoneyFlowRatio) >= 1 && Number(r.buyerPower) > 1)
    .sort((a, b) =>
      (Number(b.realMoneyFlowRatio) * Number(b.buyerPower)) -
      (Number(a.realMoneyFlowRatio) * Number(a.buyerPower))
    )
    .slice(0, limit);

  const enriched = [];
  for (const row of candidates) {
    const result = await fetchDailyHistory(row.insCode, 55);
    enriched.push(result.history.length ? enrichFromHistory(row, result.history) : row);
  }

  return enriched;
}

function buildResult(best, diagnostics) {
  const verified =
    !!best &&
    best.reachable &&
    !best.blocked &&
    Number(best.validMarketRows) >= 500;

  return {
    status: verified ? "LIVE_VERIFIED" : "LIVE_NOT_VERIFIED",
    verified,
    source: best?.source || null,
    receivedAt: new Date().toISOString(),
    marketDate: null,
    symbolCount: Number(best?.validMarketRows || 0),
    latestHeven: best?.latestHeven ?? null,
    rows: best?.rows || [],
    diagnostics
  };
}

async function fetchSnapshot() {
  const url = process.env.TSETMC_SNAPSHOT_URL || DEFAULT_SNAPSHOT_URL;
  const started = Date.now();
  try {
    const result = await fetchText(url, "application/json");
    if (!result.ok) return { ok: false, error: `HTTP ${result.status}`, elapsedMs: Date.now() - started };
    const snapshot = JSON.parse(result.raw);
    const capturedAt = new Date(snapshot.generatedAt || snapshot.receivedAt || 0).getTime();
    const ageMs = Date.now() - capturedAt;
    const fresh = Number.isFinite(capturedAt) && ageMs >= 0 && ageMs <= SNAPSHOT_MAX_AGE_MS;
    const valid = snapshot?.status === "LIVE_VERIFIED" && snapshot?.verified === true &&
      Number(snapshot?.symbolCount) >= 500 && Array.isArray(snapshot?.rows) && snapshot.rows.length >= 500;
    return {
      ok: fresh && valid,
      fresh,
      valid,
      ageMs,
      source: snapshot.source || "github-actions-tsetmc-snapshot",
      snapshot,
      elapsedMs: Date.now() - started,
      endpoint: url
    };
  } catch (error) {
    return {
      ok: false,
      elapsedMs: Date.now() - started,
      endpoint: url,
      error: error instanceof Error ? error.message : "Snapshot fetch failed"
    };
  }
}

export async function collectLiveMarket() {
  if (process.env.TSETMC_USE_SNAPSHOT !== "0" && process.env.VERCEL) {
    const snapshot = await fetchSnapshot();
    if (snapshot.ok) {
      return {
        ...snapshot.snapshot,
        source: snapshot.source,
        receivedAt: snapshot.snapshot.receivedAt || snapshot.snapshot.generatedAt,
        diagnostics: [{
          source: snapshot.source,
          reachable: true,
          validMarketRows: snapshot.snapshot.symbolCount,
          blocked: false,
          latestHeven: snapshot.snapshot.latestHeven ?? null,
          elapsedMs: snapshot.elapsedMs,
          endpoint: snapshot.endpoint,
          snapshotAgeMs: snapshot.ageMs,
          snapshotFresh: true
        }]
      };
    }
  }

  const legacy = await collectLegacy();
  if (legacy.validMarketRows >= 500 && legacy.reachable && !legacy.blocked) {
    let enrichedRows = await enrichCandidates(legacy.rows, 30);
    enrichedRows = await enrichValuation(enrichedRows, 20);
    return {
      ...buildResult({ ...legacy, rows: enrichedRows }, [
        {
          source: legacy.source,
          validMarketRows: legacy.validMarketRows,
          latestHeven: legacy.latestHeven,
          elapsedMs: legacy.elapsedMs,
          endpoint: legacy.endpoint,
          diagnostics: legacy.diagnostics,
          enrichedCandidates: enrichedRows.length
        }
      ]),
      enrichment: {
        type: "daily-history",
        candidates: enrichedRows.length,
        volumeAvg30d: true,
        sma20: true,
        sma50: true,
        resistance20d: true,
        support20d: true,
        volatilityPct30d: true,
        valuation: true
      }
    };
  }

  const cdn = await collectCdn();
  const candidates = [legacy, cdn]
    .filter(x => x && Number(x.validMarketRows) > 0)
    .sort((a, b) => Number(b.validMarketRows) - Number(a.validMarketRows));
  const best = candidates[0] || legacy || cdn;

  let enrichedRows = best?.rows ? await enrichCandidates(best.rows, 30) : [];
  enrichedRows = await enrichValuation(enrichedRows, 20);
  return {
    ...buildResult(best ? { ...best, rows: enrichedRows } : best, [legacy, cdn].map(x => ({
      source: x.source,
      reachable: x.reachable,
      validMarketRows: x.validMarketRows,
      blocked: x.blocked,
      latestHeven: x.latestHeven ?? null,
      elapsedMs: x.elapsedMs,
      endpoint: x.endpoint || null,
      error: x.error || null,
      diagnostics: x.diagnostics || null
    }))),
    enrichment: {
      type: "daily-history",
      candidates: enrichedRows.length,
      volumeAvg30d: true,
      sma20: true,
      sma50: true,
      resistance20d: true
    }
  };
}
